...
def get_entity_relations_via_type_relation(dao, set_news_id: List[str], entity_id: str, type_relation:str, type_entity:str):
    query = " PROFILE " + \
    "MATCH (entity:" + type_entity + "{entityID:$id_entity})-[:HAS_SUBJECT|HAS_OBJECT]-(facts:Fact{relation: $type_relation})" + \
    "WITH facts " + \
    "MATCH (facts)-[rel]-(entity)" + \
    "RETURN facts, rel, entity"

    result = dao.run_read_query(query, {"type_relation": type_relation.replace(" ", "_").upper(), "id_entity": entity_id})
    return result
