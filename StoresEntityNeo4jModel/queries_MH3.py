...
def get_all_relations_in_set_news(dao, set_news_id: List[str], entity_id: str, type_relation:str, type_entity:str):
    ...
    MATCH (news:News{entityID:news_id})-[:HAS_FACT]->(facts)-[rel]->(entity)
...
def get_entity_relations_in_set_news(dao, set_news_id: List[str], entity_id: str, type_relation:str, type_entity:str):
    ...
    MATCH (news:News{entityID:news_id})-[:HAS_FACT]->(facts)-[]->({entityID:$id_entity})
    WITH facts
...
def get_entity_relations_via_type_relation(dao, set_news_id: List[str], entity_id: str, type_relation:str, type_entity:str):
    query = " PROFILE " + \
    "MATCH (entity:" + type_entity + "{entityID:$id_entity})-[:HAS_SUBJECT_" + type_relation.replace(" ", "_").upper() + \
            "|:HAS_OBJECT_" + type_relation.replace(" ", "_").upper() + "]-(facts) " + \
    "WITH facts " + \
    "MATCH (facts)-[rel]-(entity)" + \
    "RETURN facts, rel, entity"

    result = dao.run_read_query(query, {"type_relation": type_relation, "id_entity": entity_id, "type_entity": type_entity})
    return result
